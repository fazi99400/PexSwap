...
contract PexSwapPair is PexSwapERC20 {
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        uint256 balance0 = IPXC20(token0).balanceOf(address(this));
        uint256 balance1 = IPXC20(token1).balanceOf(address(this));
    ...
    function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
        ...
        uint256 balance0 = IPXC20(_token0).balanceOf(address(this));
        uint256 balance1 = IPXC20(_token1).balanceOf(address(this));
    ...
    function skim(address to) external lock {
        address _token0 = token0;
        address _token1 = token1;
        _safeTransfer(_token0, to, IPXC20(_token0).balanceOf(address(this)) - reserve0);
        _safeTransfer(_token1, to, IPXC20(_token1).balanceOf(address(this)) - reserve1);
    }

    // force reserves to match balances
    function sync() external lock {
        _update(
            IPXC20(token0).balanceOf(address(this)),
            IPXC20(token1).balanceOf(address(this)),
            reserve0,
            reserve1
        );
    }
