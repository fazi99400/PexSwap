...
contract WPEX {
    ...
    function withdraw(uint256 wad) public {
        require(balanceOf[msg.sender] >= wad, "WPEX: INSUFFICIENT_BALANCE");
        balanceOf[msg.sender] -= wad;
        (bool ok, ) = msg.sender.call{value: wad}("");
        require(ok, "WPEX: PEX_TRANSFER_FAILED");
    ...
    function transferFrom(address src, address dst, uint256 wad) public returns (bool) {
        require(balanceOf[src] >= wad, "WPEX: INSUFFICIENT_BALANCE");
        if (src != msg.sender && allowance[src][msg.sender] != type(uint256).max) {
            require(allowance[src][msg.sender] >= wad, "WPEX: INSUFFICIENT_ALLOWANCE");
