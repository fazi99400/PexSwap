//! Instruction processing for the PexSwap Rust program.
//!
...
use crate::error::PexSwapError;
use crate::instruction::PexSwapInstruction;
use crate::math;
...
impl Processor {
    pub fn process(
        ...
        let ix = PexSwapInstruction::try_from_slice(instruction_data)
            .map_err(|_| ProgramError::InvalidInstructionData)?;

        match ix {
            PexSwapInstruction::InitializePool => {
                msg!("PexSwap: InitializePool");
                Self::initialize_pool(program_id, accounts)
            }
            PexSwapInstruction::AddLiquidity {
                amount0_desired,
                amount1_desired,
                amount0_min,
                amount1_min,
            } => {
                msg!("PexSwap: AddLiquidity");
                Self::add_liquidity(
                    program_id,
                    accounts,
                    amount0_desired,
            ...
            PexSwapInstruction::RemoveLiquidity {
                liquidity,
                amount0_min,
                amount1_min,
            } => {
                msg!("PexSwap: RemoveLiquidity");
                Self::remove_liquidity(program_id, accounts, liquidity, amount0_min, amount1_min)
            }
            PexSwapInstruction::SwapExactIn {
                amount_in,
                min_amount_out,
                zero_for_one,
            } => {
                msg!("PexSwap: SwapExactIn");
                Self::swap_exact_in(program_id, accounts, amount_in, min_amount_out, zero_for_one)
            }
            PexSwapInstruction::SwapExactOut {
                amount_out,
                max_amount_in,
                zero_for_one,
            } => {
                msg!("PexSwap: SwapExactOut");
    ...
    fn initialize_pool(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
        ...
        if token0_mint.key == token1_mint.key {
            return Err(PexSwapError::IdenticalTokens.into());
        }
        ...
        if let Ok(p) = existing {
            if p.is_initialized {
                return Err(PexSwapError::AlreadyInitialized.into());
            }
        }
    ...
    fn add_liquidity(
        ...
        if !pool.is_initialized {
            return Err(PexSwapError::NotInitialized.into());
        }
        ...
        let (amount0, amount1) = if pool.reserve0 == 0 && pool.reserve1 == 0 {
            ...
            let amount1_optimal = math::quote(amount0_desired, pool.reserve0, pool.reserve1)
                .ok_or(PexSwapError::Overflow)?;
            if amount1_optimal <= amount1_desired {
                if amount1_optimal < amount1_min {
                    return Err(PexSwapError::SlippageExceeded.into());
                }
        ...
        .ok_or(PexSwapError::InsufficientLiquidity)?;
        if liquidity == 0 {
            return Err(PexSwapError::InsufficientLiquidity.into());
        }
        ...
        pool.reserve0 = pool.reserve0.checked_add(amount0).ok_or(PexSwapError::Overflow)?;
        pool.reserve1 = pool.reserve1.checked_add(amount1).ok_or(PexSwapError::Overflow)?;
        pool.lp_total_supply = pool
            .lp_total_supply
        ...
        msg!("PexSwap: minted {} LP", liquidity);
        Ok(())
    }
    ...
    fn remove_liquidity(
        ...
        pool.reserve0 = pool.reserve0.checked_sub(amount0).ok_or(PexSwapError::Overflow)?;
        pool.reserve1 = pool.reserve1.checked_sub(amount1).ok_or(PexSwapError::Overflow)?;
        pool.lp_total_supply = pool
            .lp_total_supply
        ...
        msg!("PexSwap: burned {} LP", liquidity);
        Ok(())
    }
    ...
    fn swap_exact_in(
        ...
        msg!("PexSwap: swapped {} in for {} out", result.amount_in, result.amount_out);
        Ok(())
    }
    ...
    fn swap_exact_out(
        ...
        msg!("PexSwap: paid {} in for exact {} out", amount_in, amount_out);
        Ok(())
    }
    ...
    fn settle_swap<'a>(
        ...
        let adj_in = new_reserve_in
            ...
            .ok_or(PexSwapError::Overflow)?
            .checked_sub(amount_in.checked_mul(3).ok_or(PexSwapError::Overflow)?)
            .ok_or(PexSwapError::Overflow)?;
        let adj_out = new_reserve_out
            .checked_mul(math::FEE_DENOMINATOR)
            .ok_or(PexSwapError::Overflow)?;
        let lhs = adj_in.checked_mul(adj_out).ok_or(PexSwapError::Overflow)?;
        let rhs = reserve_in_before
            ...
            .ok_or(PexSwapError::Overflow)?;
        if lhs < rhs {
            return Err(PexSwapError::InvariantK.into());
        }
